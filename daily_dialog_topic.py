...
def convert_to_json(filename):
    ...
    def random_select_conversation(all_dialogs):
        ...
        random_selected_emotions = ['<'+item['emotion']+'>' for item in random_selected]
    ...
    for conversation in all_dialogs:
        ...
        for i in range(len(conversation)-1):
            ...
            utterance["emotion"] = ['<'+conv['emotion']+'>' for conv in conversation[:i+1]]
            ...
            true_emotion = '<' + conversation[i + 1]['emotion'] + '>'
        ...
        if utterances:
            ...
            dataset["train"].append({"topic": "<"+topic+">", "utterances": utterances})
...
def merge_dicts():
    with open("/home/rohola/data/daily_dialog_topic/test_out.json") as file_reader:
        test_dict = json.load(file_reader)

    test_dict["valid"] = test_dict["train"]
    del test_dict["train"]
    with open("/home/rohola/data/daily_dialog_topic/valid_out.json") as file_reader:
        valid_dict = json.load(file_reader)

    test_dict["valid"].extend(valid_dict["train"])

    with open("/home/rohola/data/daily_dialog_topic/train_out.json") as file_reader:
        train_dict = json.load(file_reader)

    dataset_dict = {**train_dict, **test_dict}

    with open("/home/rohola/data/daily_dialog_topic/daily_dialog.json", 'w') as file_writer:
...
if __name__ == "__main__":
    ...
    #
    # with open('/home/rohola/data/daily_dialog_topic/valid_out.json', 'w') as fp:
